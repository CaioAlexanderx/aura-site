...
## 4. Fase 3 — Checkout Trial `/comecar` ⬜ (próxima)
...
#### T3.1 ⚠️ [Opus] — Backend: garantir trial 7d no plano Negócio
O register hoje não dá o trial/plano que queremos. **Resolver assim (preferência: sem deploy de código):**
...
#### T3.2 ⚠️ [Opus] — Definir handoff de sessão pós-register
Após `register` retornar o JWT, o usuário precisa **entrar logado no app** (`app.getaura.com.br`).
1. Investigar no `aura-app` como o app aceita uma sessão externa (query param com token? deep link? cookie? tela de login com email pré-preenchido?).
2. Definir o mecanismo e implementar em `js/comecar.js`. Fallback aceitável p/ v1: redirecionar pra tela de login do app com o e-mail pré-preenchido.
